...
pub fn user_cow_int(task: Arc<UserTask>, cx_ref: &mut TrapFrame, vaddr: VirtAddr) {
    ...
    /*warn!(
        "Processing page fault for vaddr: {:#x}, vaddr.floor(): {:#x}",
        vaddr.raw(),
        vaddr.floor().raw()
    );*/
    ...
    if let Some(area) = area {
        /*warn!("Found existing memory area for vaddr: {:#x}", vaddr.raw());
                warn!(
                    "Area details: start={:#x}, len={:#x}, offset={:#x}, mtype={:?}, has_file={}",
                    area.start,
                    area.len,
                    area.offset,
                    area.mtype,
        ...
        */
        ...
        let ppn = match finded {
            ...
            None => {
                ...
                /*  warn!(
                    "Calculated file_offset: {:#x} for vaddr: {:#x}",
                    file_offset,
                    vaddr.raw()
                );*/
        ...
        if let Some((mapped_paddr, mapped_flags)) = task.page_table.translate(vaddr.floor()) {
            /*  warn!(
                "Mapping verification: vaddr {:#x} -> paddr {:#x}, flags {:?}",
                vaddr.floor().raw(),
                mapped_paddr.raw(),
                mapped_flags
            );*/
        ...
        // 新增：处理低地址区域（如 0x10690）
            ...
            /*warn!(
                "Detected CodeSection range access for vaddr: {:#x}",
                vaddr.raw()
            );
            warn!(
                "Attempting to handle CodeSection page fault for vaddr: {:#x}",
                vaddr.raw()
            );*/
            ...
            if !has_mapping {
                /*warn!(
                    "No ELF segment mapping found for low address vaddr: {:#x}",
                    vaddr.raw()
                );
                warn!("Available memory areas:");*/
                for (i, area) in task.pcb.lock().memset.iter().enumerate() {
                    /*warn!(
                        "  Area {}: start={:#x}, len={:#x}, type={:?}, has_file={}",
                        i,
                        area.start,
                        area.len,
                        area.mtype,
            ...
            // 添加详细的调试信息
            /*warn!(
                "Before get_elf_segment_for_addr: task_id={}, process_id={}, PCB arc count={}",
                elf_lookup_task.task_id,
                elf_lookup_task.process_id,
                Arc::strong_count(&elf_lookup_task.pcb)
            );*/
            ...
            if let Some((file, file_offset, _)) = elf_lookup_task.get_elf_segment_for_addr(vaddr) {
                ...
                if file_offset >= file_size {
                    /*warn!(
                        "File offset {:#x} exceeds file size {:#x} for vaddr: {:#x}",
                        file_offset,
                        file_size,
                        vaddr.raw()
                    );*/
                ...
                if let Some(ppn) = task.frame_alloc(vaddr.floor(), MemType::CodeSection, page_count)
                {
                    ...
                    if read_size > 0 {
                        ...
                        /*warn!(
                            "No data to read at offset {:#x} for vaddr: {:#x}",
                            file_offset,
                            vaddr.raw()
                        );*/
                    }
            ...
            /*warn!(
                "Falling back to blank page allocation for vaddr: {:#x}",
                vaddr.raw()
            );*/
            let page_count = 1;
            if let Some(_) = task.frame_alloc(vaddr.floor(), MemType::CodeSection, page_count) {
                /*warn!(
                    "Successfully allocated blank CodeSection for vaddr: {:#x}",
                    vaddr.raw()
                );*/
                return;
            } else {
                /*warn!(
                    "Failed to allocate CodeSection for vaddr: {:#x}",
                    vaddr.raw()
                );*/
            }
        }
        // 新增：处理堆区域扩展
            ...
            warn!(
                "Attempting to allocate Mmap (heap) for vaddr: {:#x}",
                vaddr.raw()
            );*/
            let heap_page_count = 1;
            if let Some(_) = task.frame_alloc(vaddr.floor(), MemType::Mmap, heap_page_count) {
                /*warn!(
                    "Successfully allocated Mmap (heap) for vaddr: {:#x}",
                    vaddr.raw()
                );*/
                return;
            } else {
                /*warn!(
                    "Failed to allocate Mmap (heap) for vaddr: {:#x}",
                    vaddr.raw()
                );*/
            }
            ...
            unsafe {
                if LAST_TLS_ADDR == vaddr.floor().raw() {
                    ...
                    if TLS_COUNT > 3 {
                        /*warn!(
                            "TLS loop detected at {:#x}, exiting task {}",
                            vaddr.raw(),
                            task.get_task_id()
                        );*/
                        task.exit(1);
...
impl UserTaskContainer {
    pub async fn handle_syscall(&self, cx_ref: &mut TrapFrame) -> UserTaskControlFlow {
        /*warn!(
            "Task {} entering handle_syscall, SEPC: {:#x}",
        ...
        if matches!(escape_reason, EscapeReason::SysCall) {
            ...
            if syscall_id == Sysno::rt_sigreturn.id() as _ {
                /*  warn!(
                    "Task {} rt_sigreturn, returning Break",
                    self.task.get_task_id()
                );*/
            ...
            /* warn!(
                "Task {} non-syscall escape reason: {:?}",
                self.task.get_task_id(),
                escape_reason
            );*/
        }
        ...
        /*  warn!(
            "Task {} handle_syscall returning Continue",
            self.task.get_task_id()
        );*/
